# Everything is Big (70 pts)
crypto

## Deskripsi
> We have a supercomputer at work, so I've made sure my encryption is secure by picking massive numbers!
>
> Challenge files:
> - [source.py](https://cryptohack.org/static/challenges/source_a9eaeb4b534011e36229bf45be4ea455.py)
> - [output.txt](https://cryptohack.org/static/challenges/output_1884d0ba92d19ad074549e174cdf5a70.txt)

## Solusi
Diberikan nilai n, e, dan ct. disini nilai e yang diberikan sangatlah besar. namun, perlu diketahui bahwa
nilai e yang baik harusnya tidak besar, karena jika e besar maka d kecil dan jika e kecil maka d besar. Ketika nilai 
d kecil, d rentan di pecahkan dengan [wiener's attack](https://en.wikipedia.org/wiki/Wiener%27s_attack).

kita bisa memanfaatkan library ```owiener``` untuk melakukan serangan ini.
``` python
import owiener
from Crypto.Util.number import long_to_bytes

N = 
e = 
c = 
d = owiener.attack(e, N)

print(long_to_bytes(pow(c, d, N)).decode())
```
